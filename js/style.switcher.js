...
var styleSwitcher = {
	...
	initialize: function() {
		...
		// Style Switcher CSS
		$("head").append($('<link rel="stylesheet">').attr("href", theme_settings.assets + "master/style-switcher/style-switcher.css") );
		$("head").append($('<link rel="stylesheet/less">').attr("href", theme_settings.assets + "master/less/skin.less") );
		$("head").append($('<link rel="stylesheet">').attr("href", theme_settings.assets + "master/style-switcher/colorpicker/css/colorpicker.css") );

		$.getScript( theme_settings.assets + "master/style-switcher/colorpicker/js/bootstrap-colorpicker.js", function(data, textStatus, jqxhr) {

			less = {
				env: "development"
			};

			$.getScript( theme_settings.assets + "master/less/less.js", function(data, textStatus, jqxhr) {
		...
		$.getScript(theme_settings.assets + "master/style-switcher/cssbeautify/cssbeautify.js", function(data, textStatus, jqxhr) {});
				...
				$("<div />")
					...
					.append(
						...
						$("<div />")
							...
							.append(
								$("<a />")
									.attr("href", "http://spyropress.com/porto/")
									.attr("data-website-type", "normal")
									.html("Normal"),
								$("<a />")
									.attr("href", "http://spyropress.com/porto/one-page-website/")
		...
		$.each(patterns, function(key, value) {
			...
			var pattern = $("<li />")
							.append(
								$("<a />")
									.addClass("pattern")
									.css("background-image", "url(" + theme_settings.assets + "img/patterns/" + value + ".png)")
									.attr({
										"data-pattern": value,
										"href": "#",
	...
	setColor: function(color) {
		...
		if(color == this.container.find("ul[data-type=colors] li:first a").attr("data-color-hex")) {
			$("h1.logo img").attr("src", theme_settings.assets + "img/logo-default.png");
		} else {
			$("h1.logo img").attr("src", theme_settings.assets + "img/logo.png");
		}
	...
	setLayoutStyle: function(style, refresh) {
		...
		if(style == "wide") {
			...
			$("link[href*='bootstrap-responsive']").attr("href", theme_settings.assets + "css/bootstrap-responsive.css");
			...
			$("body").addClass("boxed");
			$("link[href*='bootstrap-responsive']").attr("href", theme_settings.assets + "css/bootstrap-responsive-boxed.css");
	...
	setPattern: function(pattern) {
		...
		if(isBoxed) $("body").css("background-image", "url(" + theme_settings.assets + "img/patterns/" + pattern + ".png)")
	...
	getCss: function() {
		...
		if(isBoxed) {
			raw = 'body { background-image: url("' + theme_settings.assets + 'img/patterns/' + $.cookie("pattern") + '.png"); }';
